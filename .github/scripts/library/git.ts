...
export async function getChangedFiles() {
    ...
    const diffResults = await execAsyncOrThrow(`git --no-pager diff --name-only ${baseRef.sha}`);
    return diffResults.stdout.split('\n')
}

export async function readFileAtBase(file: string) {
    const baseRef = await getBaseRef();
    const original = await execAsyncOrThrow(`git --no-pager show ${baseRef.sha}:${file}`)
